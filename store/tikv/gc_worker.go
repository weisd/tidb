...
const (
	gcWorkerTickInterval = time.Second * 5
	gcWorkerLease        = gcWorkerTickInterval * 2
	gcRunInterval        = time.Second * 20
	gcLeaderUUIDKey      = "tikv_gc_leader_uuid"
	gcLeaderLeaseKey     = "tikv_gc_leader_lease"
	gcLeaderLeaseFormat  = "20060102-15:04:05 -0700 MST"
	gcLastRunKey         = "tikv_gc_last_run"
)
...
func (w *GCWorker) resolveLocks(safePoint uint64) error {
	...
	log.Infof("[gc worker] %s finish resolve locks, safePoint: %v, total resolved: %v, cost time: %s", w.uuid, safePoint, totalResolvedLocks, time.Now().Sub(startTime))
	return nil
}
...
func (w *GCWorker) doGC(safePoint uint64) error {
	...
	log.Infof("[gc worker] %s finish gc, safePoint: %v, cost time: %s", w.uuid, safePoint, time.Now().Sub(startTime))
	return nil
}
...
func (w *GCWorker) loadLease() (time.Time, error) {
	...
	log.Debugf("[gc worker] got lease: %s", lease)
	return lease, nil
}
